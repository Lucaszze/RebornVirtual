// ---------------------------------------------------------------------------
// A falha de rastreamento como medida, e não como anedota.
//
...
// instante, a conta de onde está. É o mesmo sintoma da Seção 11, item 3 — a
// bancada ancorada que congela quando a câmera aponta para uma parede lisa —, e
// é o único que se lê de dentro do código, sem sensor extra.
//
// O contador é deliberadamente burro: conta quadros com pose e quadros sem, e
// mais nada. A interpretação fica na função de diagnóstico, separada de
// propósito — misturar contagem e julgamento produz medidor que sempre concorda
// com quem o escreveu.
// ---------------------------------------------------------------------------
...
/**
 * Acumula a contagem quadro a quadro. O parâmetro `visivel` separa perda de
 * rastreamento de sessão fora de foco: quando quem usa abre o menu do sistema do
 * visor, a sessão continua viva, os quadros continuam chegando e a pose deixa de
 * vir — e contar isso como falha de sensor acusaria o aparelho de um defeito que
 * é comportamento normal do sistema.
 */
...
/**
 * Traduz a contagem numa frase, com as causas que a bibliografia aponta para
 * degradação de rastreamento óptico: superfície sem textura, iluminação pobre e
 * movimento brusco. A frase não afirma qual das três aconteceu — daqui não se
 * distingue, e escolher uma seria inventar o diagnóstico junto com a medida.
 */
export function diagnosticar(estabilidade: Estabilidade): string {
  ...
  return (
    ...
    'sem textura, iluminação pobre ou movimento brusco — e daqui não se distingue qual.'
  );
}
